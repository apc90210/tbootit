...
def test_no_destructive_database_calls_in_tests():
    """Verify that no unit test file contains drop_all or DROP TABLE statements."""
    search_dirs = [
        "tests",
        "/app/tests",
        ...
        if os.path.exists(d):
            for py_file in glob.glob(os.path.join(d, "*.py")):
                with open(py_file, "r", encoding="utf-8", errors="ignore") as f:
                    content = f.read()
                    assert "drop_all" not in content, f"Found forbidden 'drop_all' call in {py_file}"
...
def test_engine_url_is_strictly_isolated():
    ...
    assert "isolated_test.db" in url_str or ":memory:" in url_str or "test" in url_str
