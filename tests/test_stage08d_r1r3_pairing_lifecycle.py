...
def test_fresh_code_redeem_success():
    ...
    assert status_data["version"] in ("0.2.54", "0.2.55", "0.2.56", "0.2.57", "0.2.58", "0.2.59", "0.2.60", "0.2.61", "0.2.62")
...
def test_extension_package_version_and_dynamic_origin():
    """Verify Chrome extension source files include dynamic origin support, persistence, and v0.2.59."""
    ...
    assert manifest["version"] in ("0.2.56", "0.2.57", "0.2.58", "0.2.59", "0.2.60", "0.2.61", "0.2.62", "0.2.63", "0.2.64")
    if manifest["version"] < "0.2.63":
        assert "https://144.31.50.134/*" in manifest["host_permissions"]
    else:
        assert "https://localhost:8443/*" in manifest["host_permissions"]
        assert "https://144.31.15.88/*" in manifest["host_permissions"]
        assert "https://144.31.50.134/*" not in manifest["host_permissions"]
    ...
    assert any(v in sw_code for v in ("0.2.56", "0.2.57", "0.2.58", "0.2.59", "0.2.60", "0.2.61", "0.2.62", "0.2.63", "0.2.64"))
    ...
    assert any(f"v{v}" in popup_html for v in ("0.2.56", "0.2.57", "0.2.58", "0.2.59", "0.2.60", "0.2.61", "0.2.62", "0.2.63", "0.2.64"))
    ...
    assert any(f"v{v}" in tmpl for v in ("0.2.56", "0.2.57", "0.2.58", "0.2.59", "0.2.60", "0.2.61", "0.2.62", "0.2.63", "0.2.64"))
