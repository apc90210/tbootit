...
def test_fresh_code_redeem_success():
    ...
    assert status_data["version"] in ("0.2.54", "0.2.55", "0.2.56", "0.2.57", "0.2.58")
...
def test_extension_package_version_and_dynamic_origin():
    ...
    assert manifest["version"] in ("0.2.56", "0.2.57", "0.2.58")
    ...
    assert any(v in sw_code for v in ("0.2.56", "0.2.57", "0.2.58"))
    ...
    assert any(f"v{v}" in popup_html for v in ("0.2.56", "0.2.57", "0.2.58"))
    ...
    assert any(f"v{v}" in tmpl for v in ("0.2.56", "0.2.57", "0.2.58"))
